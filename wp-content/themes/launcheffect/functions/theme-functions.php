...
function check_mobile () {
	...
	if ($iphone || $android || $ipod || $ipad || $blackberry) { 
		return true;
	} else {
		return false;
	}
...
function get_thumbnail($img_url) {
	global $wpdb;
	$detect = new Mobile_Detect;
	if ( $detect->isMobile() && !$detect->isTablet()) {
		$image = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->posts WHERE guid = %s", $img_url ) );
		return $url = wp_get_attachment_image_src($image->ID, 'page-thumbnail')[0];
	} else {
		return $img_url;
	}
}	