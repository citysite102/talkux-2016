...
class LE_Stats_Admin_Page extends LE_Admin_Page {
	...
	function build_le_stats_page(){
		...
		if ( isset($_REQUEST['action']) || isset( $_REQUEST['action2']) ) {
			...
			switch($_REQUEST['action']) {
				case "trash":
					...
					$referred_list = $wpdb->get_results( "SELECT referred_by, count(referred_by) as count FROM {$stats_table} WHERE id $id GROUP BY referred_by" );
					// Find code of deleted id
					$deleted_list = $wpdb->get_results( "SELECT code FROM {$stats_table} WHERE id $id" );
					// Delete
					$count = is_array( $id ) ? count( $id ) : 1;
					$plurality = _n( 'These items have', 'This item has', $count, 'launcheffect' );
					$handle = $wpdb->query( "DELETE FROM $stats_table WHERE id $id" );
					$handle = 1;
					$msg = "$plurality been removed.";
					// Update referred_by
					foreach ( $referred_list as $referred ) {
						if ( ! empty( $referred->referred_by ) ){
							$conversions = (int) $wpdb->get_var( "SELECT conversions FROM $stats_table WHERE code = '$referred->referred_by'" );
							$count = (int) $referred->count;
							$new_conversions = $conversions - $count;
							$wpdb->query( "UPDATE $stats_table SET conversions = $new_conversions WHERE code = '$referred->referred_by'" );
						}
					} 
					// Remove reffered_by by deleted ID
					foreach ( $deleted_list as $deleted ) {
						$wpdb->query( "UPDATE $stats_table SET referred_by = '' WHERE referred_by = '$deleted->code'" );
					}
				break;
			}
			if ($handle) {
		...
		<div class="wrap" id="stats-wrapper">
			...
			<div>
				<?php foreach ($emails as $email) {
					$link = get_home_url().'/?ref='.$email->code;
				} ?>
				<h3> <?php _e('Referral Link:', 'launcheffect'); ?> 
