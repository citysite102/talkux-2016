...
function le_submit() {
	...
	$different_notify = get_option('lefx_different_notify');
		...
		if ($postEmail = is_email($_POST['email'])) {
			...
			if ($count > 0) {
				...
				if (!count($required)) {
					...
					if ($auto_responder||$admin_notify||$different_notify) {
					...
					// Prepare notify email
					if ($admin_notify||$different_notify) {
					...
					if ($admin_notify) {
						...
						$emailed = @wp_mail( $admin_email, html_entity_decode($subject, ENT_QUOTES), stripslashes($copy), $headers );
					}
					// Another admin notifier
					if ($different_notify) {
						$different_notify_email = get_option('lefx_different_notify_email');
						$emailed = @wp_mail( $different_notify_email, html_entity_decode($subject, ENT_QUOTES), stripslashes($copy), $headers );	
					}
...
================================================== */
	...
	html,
	body {
		background: <?php echo ($pg_bg_color = lercolor('page_background_color')); ?>;
	}
	...
	<?php if ( $supersize = leimg('supersize', 'supersize_disable', 'plugin_options')) : ?> 
		#background {
			background-image: url('<?php echo $supersize; ?>');
	...
	<?php $size = get_option('container_width');
	if ($size == 'large'):
		$max_width = '768px';
	elseif ($size == 'medium') :
		$max_width = '590px';
	...
	@media screen and (max-width: <?php echo $max_width; ?>) {	
		body.page-template-launch #background {
			background-image: url('<?php echo get_thumbnail($supersize); ?>');
			position: relative; 
			height: 33%; 
			<?php if ($pg_bg_color == 'transparent' && empty( $supersize )) : ?>
				height: 0;
			<?php endif; ?>
		}
	}
	...
	@media screen and (max-width: 768px) {	
		body:not(.page-template-launch) #background {
			background-image: url('<?php echo get_thumbnail($supersize); ?>');
			position: relative; 
			height: 33%;
			<?php if ($pg_bg_color == 'transparent' && empty( $supersize )) : ?>
	...
	@media screen and (min-width: 768px) {	
		.page-template-launch>#background {
			background-image: url('<?php echo $supersize; ?>');
			background-repeat: no-repeat;
			-webkit-background-size: cover;
			-moz-background-size: cover;
