...
<body <?php body_class((lefx_version() != 'premium')?"lite":""); ?>>
	...
	?>>
		<?php if ( $slides_enabled && ($img_url = $supersize) ) {
			$img_url = get_thumbnail($img_url);
			$output = "<div style='background-image: url($img_url);'></div>";
			for ( $i=2;$i<6;$i++) {
				if ($img_url = get_option('lefx_bg_image'.$i)) {
					$img_url = get_thumbnail($img_url);
